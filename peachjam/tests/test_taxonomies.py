...
class TaxonomyTestCase(WebTest):
    ...
    def test_restricted_taxonomy_unauthorized(self):
        ...
        land_url = reverse(
            "taxonomy_detail",
            kwargs={"topic": land.get_root().slug, "child": land.slug},
        )
        response = self.app.get(land_url, user=unauthorized_user)
        self.assertEqual(response.status_code, 200)
        self.assertNotIn("Climate Change", response.text)

        environment = Taxonomy.objects.get(name="Environment")
        environment_url = reverse(
            "taxonomy_detail",
            kwargs={"topic": environment.get_root().slug, "child": environment.slug},
        )
        response = self.app.get(
            environment_url, user=unauthorized_user, expect_errors=True
        )
    ...
    def test_restricted_taxonomy_authorized(self):
        ...
        response = self.app.get(land_url, user=authorized_user)
        self.assertEqual(response.status_code, 200)
        ...
        response = self.app.get(environment_url, user=authorized_user)
        self.assertEqual(response.status_code, 200)
